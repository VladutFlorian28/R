# Script R: Estimarea numărului de cărți citite pe baza vârstei și a duratei calității de membru
# Urmează pașii din roadmap: citire date, EDA, modelare (regresie + clasificare), predicții, export
# Autor: (tu)
# Fișier necesar în directorul de lucru: "library_books_read.csv"

# --- 0. Setup ------------------------------------------------------------------
library(tidyverse)   # include readr, dplyr, ggplot2
library(broom)       # tidy() pentru modele

set.seed(123)

# --- 1. Citește datele --------------------------------------------------------
# Așteptăm un fișier cu cel puțin coloanele: member_age, membership_years, books_read
...
# Dacă există NA, alegem o strategie simplă: eliminare rânduri incomplete
if(any(na_counts > 0)){
...
# --- 2. Explorare vizuală (EDA) -----------------------------------------------
# Scatter: books_read vs member_age
...
# Scatter: books_read vs membership_years
...
# Dacă vrei, salvează graficele ca fișiere PNG
...
# --- 3. Modelare (regresie liniară): prezic numărul de cărți -------------------
# Model cu ambii predictori
...
# Predicții pe setul de antrenament (pentru evaluare)
df <- df %>% mutate(
...
# --- 5. Clasificare: identificare "cititori pasionați" -----------------------
# Definim pragul (ex.: >25 cărți)
threshold <- 25
...
# Salvăm predicțiile de clasificare
...
# --- 7. Concluzii automate (sumar text) --------------------------------------
# Folosește coeficienții pentru a spune succint care predictor e mai puternic
